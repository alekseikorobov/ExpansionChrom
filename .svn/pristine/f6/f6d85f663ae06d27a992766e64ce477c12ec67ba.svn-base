...
function initialization(){
	...
	data = getMailAdreses(window.document.body.innerHTML);
    this.port.postMessage({method:'MyCallBack', data:data});
    //window.popup = new popupObj();
}
function getMailAdreses(html)
{
	var reg = new RegExp("[A-z][A-z\\-0-9]*?@[A-z\\-0-9]+\\.[A-z\\.]+","g");
	var myArray = html.match(reg);
	if(myArray){
	var unique = myArray.filter( function(value, index, self) { 
